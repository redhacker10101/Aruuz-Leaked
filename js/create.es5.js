...
var Notification = function Notification(idlist, textlist, linklist) {
...
function notificationOpened() {
    ...
    $("#notificatioDropDown > a").each(function (index, elem) {
...
function MessageOpened() {
    ...
    var stringArray = new Array();
    $("#messageDropDown > a").each(function (index, elem) {
        stringArray[index] = elem.id;
    });
...
function taqtiClicked() {
    var changed = $(".data").each(function () {
        $(this).attr("id");
    });
...
function wordClicked(word) {
    ...
    if (word == '---') {
        var values = "<div class='modal-body urdu-naskh-medium text-right'>" + "اس لفظ کی تقطیع کے بارے میں ہمیں تحفظات ہیں۔ اگر تو اس لفظ میں املا کی اغلاط ہیں تو ان کو درست لکھیں، یا اگر  یہ لفظ کئی الفاظ کا مجموعہ ہے تو سب الفاظ کو الگ الگ لکھیں۔  " + "اگر آپ نے اوپر دیئے گئے قاعد کے مطابق لفظ لکھا اور پھر بھی سسٹم تقطیع کرنے میں ناکام رہا ہے تو اس لفظ کو ہمیں <a href= 'https://aruuz.com/discussion?id=vSZWtVktzSJvzedJ756Z'> رپورٹ</a> کریں۔" + "</div>";
        $("#myModal div.modal-body").replaceWith(values);
        $("#button-label").addClass("close-it");+$("#myModal").addClass("opened");
    } else {
        var values = {
            "text": word,
            "isChecked": false
